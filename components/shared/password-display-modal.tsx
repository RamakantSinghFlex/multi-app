...
interface PasswordDisplayModalProps {
  ...
  currentUserRole?: string | null // Add current user role
}

export function PasswordDisplayModal({ open, onOpenChange, users, currentUserRole }: PasswordDisplayModalProps) {
  ...
  // Clear passwords from localStorage when closing the modal
  const handleClose = () => {
    onOpenChange(false)
  }
