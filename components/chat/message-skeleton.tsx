...
export function MessageSkeleton({ count = 3, incoming = true }: MessageSkeletonProps) {
  return (
    <div className="space-y-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className={`flex ${incoming ? "justify-start" : "justify-end"}`}>
          <div
            className={cn("max-w-[70%] rounded-lg p-3 animate-pulse", incoming ? "bg-gray-200" : "bg-[#0c6a4a]")}
