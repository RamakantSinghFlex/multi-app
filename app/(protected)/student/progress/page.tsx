  ...
  const achievements = [
    {
      ...
      icon: <GraduationCap className="h-8 w-8 text-[#095d40]" />,
      earned: true,
      date: "2023-03-15",
    },
    {
      ...
      icon: <TrendingUp className="h-8 w-8 text-[#095d40]" />,
    ...
    },
    {
      ...
      icon: <BookOpen className="h-8 w-8 text-[#095d40]" />,
    ...
    },
    {
      ...
      icon: <Award className="h-8 w-8 text-[#095d40]" />,
      earned: false,
      date: null,
    },
  ]
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-[#858585]">Monitor your learning journey and achievements</p>
      </div>

      <div className="grid gap-6 md:grid-cols-4">
        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6">
            <GraduationCap className="mb-2 h-8 w-8 text-[#095d40]" />
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6">
            <BookOpen className="mb-2 h-8 w-8 text-[#095d40]" />
            <CardTitle className="text-sm font-medium">Subjects</CardTitle>
            <div className="mt-2 text-3xl font-bold">{Object.keys(subjectProgress).length}</div>
            <p className="text-xs text-[#858585]">Different subjects studied</p>
          </CardContent>
        </Card>

        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6">
            <TrendingUp className="mb-2 h-8 w-8 text-[#095d40]" />
            <CardTitle className="text-sm font-medium">Sessions</CardTitle>
            <div className="mt-2 text-3xl font-bold">{sessions.length}</div>
            <p className="text-xs text-[#858585]">Total tutoring sessions</p>
          </CardContent>
        </Card>

        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6">
            <Award className="mb-2 h-8 w-8 text-[#095d40]" />
      ...
      <Tabs defaultValue="subjects" className="space-y-4">
        ...
        <TabsContent value="subjects" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-medium">Subject Progress</CardTitle>
            ...
            <CardContent>
              {Object.keys(subjectProgress).length > 0 ? (
                ...
                <div className="flex h-40 flex-col items-center justify-center space-y-2">
                  ...
                  <p className="text-[#858585]">No subject progress data available yet.</p>
                </div>
              )}
            </CardContent>
          </Card>
                ...
                {achievements.map((achievement) => (
                  <Card key={achievement.id} className={`overflow-hidden ${!achievement.earned ? "opacity-60" : ""}`}>
                    <CardContent className="flex items-start gap-4 p-4">
                      <div className="rounded-full bg-[#e3fae3] p-2 text-[#095d40]">{achievement.icon}</div>
                      <div className="flex-1">
                        ...
                        {achievement.earned ? (
                          ...
                          <p className="mt-1 text-xs text-[#858585]">Not yet earned</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
