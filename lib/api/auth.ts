...
const { signin, refresh } = appClient({ name: "multi" });
...
// Google Sign-In
export async function googleSignIn(): Promise<{isError: boolean; data: any; message: string; isSuccess: boolean}> {
...
export async function getUser(emails) {
    const res = await getCurrentUser({ name: "multi" }, { fields: [emails] });
    return res;
}
